...
Workflow actions are pinned to full commit SHAs. NuGet restores run in locked mode against committed `packages.lock.json` files; refresh them with `dotnet restore SplunkSdk.slnx` when dependencies change intentionally.
